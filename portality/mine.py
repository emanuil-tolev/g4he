...
def mine_project_record(project):
    title = project.get("project", {}).get("title", "")
    abstract = project.get("project", {}).get("abstractText", "")
    impact = project.get("project", {}).get("potentialImpactText", "")
    
    return full_extract(title_text=title, title_weight=10, 
                        abstract_text=abstract, abstract_weight=5,
                        impact_text=impact, impact_weight=1)
